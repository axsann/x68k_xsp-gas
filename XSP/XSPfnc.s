...
#-------[ マスク加工 ]
	moveq.l	#-1,%d0			|# %d0.b = 255（マスクon）

0:	move.b	%d0,(%a0)+		|# マスク設定
	dbra	%d1,0b			|# 指定数処理するまでループ
...
#-------[ MFP の復活 ]
	...
	move.b	AER(%a1),%d0
	andi.b	#0b01010000,%d0
	andi.b	#0b10101111,AER(%a0)
	or.b	%d0,AER(%a0)		|# AER bit4&6 復活

	move.b	IERA(%a1),%d0
	andi.b	#0b01000000,%d0
	andi.b	#0b10111111,IERA(%a0)
	or.b	%d0,IERA(%a0)		|# IERA bit6 復活

	move.b	IERB(%a1),%d0
	andi.b	#0b01000000,%d0
	andi.b	#0b10111111,IERB(%a0)
	or.b	%d0,IERB(%a0)		|# IERB bit6 復活

	move.b	IMRA(%a1),%d0
	andi.b	#0b01000000,%d0
	andi.b	#0b10111111,IMRA(%a0)
...
A7ID	=	4			|#   スタック上 return先アドレス  [ 4 byte ]
	...
	cmp #MIN_DIVH_MIN,%d0
	bge 0f				|# #MIN_DIVH_MIN <= h なら bra
	|#-----[ MIN_DIVH_MIN > h ]
		moveq.l #MIN_DIVH_MIN,%d0	|# h = MIN_DIVH_MIN
0:
	cmp #MIN_DIVH_MAX,%d0
	ble 0f				|# #MIN_DIVH_MAX >= h なら bra
	|#-----[ MIN_DIVH_MAX < h ]
