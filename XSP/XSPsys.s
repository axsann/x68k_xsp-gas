...
CHAIN_OFS_div	= 0x30C0		|# ラスタ分割バッファ → チェイン情報 へのオフセット
...
VSYNC_INT:
	...
	move.w	#1023,0xE80012		|# ラスタ割り込み off
...
#=======[ 割り込みマスクに小細工 ]	|# 帰線期間割込み中にラスタ割込みがかかるように小細工
	...
	movea.l	#0xe88000,%a0		|# %a0.l = MFP アドレス
	move.b	IMRA(%a0),-(%a7)		|# IMRA 保存
	move.b	IMRB(%a0),-(%a7)		|# IMRB 保存
	andi.b	#0b01011110,IMRA(%a0)	|# マスクに小細工
	andi.b	#0b10110111,IMRB(%a0)	|# マスクに小細工
					|#	ビットが 0 = マスク状態
					|#	1)水平同期割り込み
					|#	2)タイマー A/B
					|#	3)垂直同期割り込み
...
#=======[ 768*512 dot mode か？ ]
	btst.b	#1,0xE80029		|# 768*512 mode なら bit1=1
	bne	VSYNC_RTE		|# 768*512 mode なら 強制終了


#=======[ スプライト表示 off ]
	bclr.b	#1,0xEB0808		|# sp_disp(0)


#=======[ ラスタ割り込みタイムチャートの指定 & 初回の割り込みの指定 ]
	movea.l	disp_struct(%pc),%a0	|# %a0.l = 表示用バッファ管理構造体アドレス

	btst.b	#4,0xE80029		|# [inside68k p.233 ]
...
_31khz:		cmpi.w	#1,buff_sp_mode(%a0)	|# buff_sp_mode == 1 か？
		...
		|#-------[ buff_sp_mode == 1 ]
			...
			bra.b	RAS_INT_init
		|#-------[ buff_sp_mode != 1 ]
0:			lea.l	XSP_chart_for_512sp_31khz(%a0),%a0
			bra.b	RAS_INT_init
...
_15khz:		cmpi.w	#1,buff_sp_mode(%a0)	|# buff_sp_mode == 1 か？
		...
		|#-------[ buff_sp_mode == 1 ]
			...
			bra.b	RAS_INT_init
		|#-------[ buff_sp_mode != 1 ]
0:			lea.l	XSP_chart_for_512sp_15khz(%a0),%a0
		|#!!	bra.b	RAS_INT_init
...
_RAS_INT_endmark:	move.l	#dummy_proc,hsyncint_sub	|#[28] 次回サブルーチンアドレス設定
			move.w	#1023,0xE80012			|#[16] 次回割り込みラスタナンバー設定


RAS_INT_init_END:
	andi.w	#0xFDFF,sr		|# 割込みマスクレベル 5（割り込み許可）
...
DISP_buff_AB:
	...
	|#-------[ buff_sp_mode != 1 の場合 ]
0:		move.w	#128*8,%d7		|# %d7.w = スプライトクリア数*8
		movea.l	#0xEB0000,%a6		|# %a6.l = スプライトクリア開始アドレス
...
#=======[ RTE ]
VSYNC_RTE:
	ori.w	#0x0700,sr		|# 割り込みマスクレベル 7（割り込み禁止）
	bsr	WAIT			|# 68030 対策
...
DISP_buff_C:
	ori.w	#0x0700,sr		|# 割込みマスクレベル 7

	movea.l	#0xe88000,%a0		|# %a0.l = MFP アドレス
	move.b	IMRA(%a0),-(%a7)		|# IMRA 保存
	move.b	IMRB(%a0),-(%a7)		|# IMRB 保存
	andi.b	#0b01011110,IMRA(%a0)	|# マスクに小細工
	andi.b	#0b11110111,IMRB(%a0)	|# マスクに小細工

	movea.l	disp_struct(%pc),%a0	|# %a0.l = 表示用バッファ管理構造体アドレス
	andi.w	#0xFDFF,sr		|# 割込みマスクレベル 5
	movea.l	div_buff(%a0),%a0		|# %a0.l = 表示用 div_buff_A アドレス
...
SP_TRANS:
	movea.l	#0xEB0000,%a6		|# %a6.l = スプライト転送先アドレス
...
SP_TRANS_div:
	...
	movea.l	0f(%pc,%d0.w),%a2		|# %a2.l = 転送数*8 別ジャンプ先
	jmp	(%a2)			|# 転送ルーチンにジャンプ
...
0:
	...
	.dcb.l	2,SP_TRANS_div_LOOP+0x3F*6	|# 0x01枚 転送
	.dcb.l	2,SP_TRANS_div_LOOP+0x3E*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x3D*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x3C*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x3B*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x3A*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x39*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x38*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x37*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x36*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x35*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x34*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x33*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x32*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x31*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x30*6	|# 

	.dcb.l	2,SP_TRANS_div_LOOP+0x2F*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x2E*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x2D*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x2C*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x2B*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x2A*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x29*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x28*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x27*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x26*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x25*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x24*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x23*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x22*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x21*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x20*6	|# 

	.dcb.l	2,SP_TRANS_div_LOOP+0x1F*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x1E*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x1D*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x1C*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x1B*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x1A*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x19*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x18*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x17*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x16*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x15*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x14*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x13*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x12*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x11*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x10*6	|# 

	.dcb.l	2,SP_TRANS_div_LOOP+0x0F*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x0E*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x0D*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x0C*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x0B*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x0A*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x09*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x08*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x07*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x06*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x05*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x04*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x03*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x02*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x01*6	|# 
	.dcb.l	2,SP_TRANS_div_LOOP+0x00*6	|# 0x40枚 転送
...
SP_CLEAR:
	...
	move.w	%d7,%d6
	andi.w	#0xFF80,%d6		|# (16sprite STEP)
...
.macro	ADJUST_DIV_Y_SUB	reg1, reg2, divy_01, divy_12, divy_23, SORT_512_1, SORT_512b_1, SORT_512_2, SORT_512b_2

							|# \reg1.w = ラスタ分割バッファ1 使用数*8
							|# \reg2.w = ラスタ分割バッファ2 使用数*8
	cmp.w	\reg1,\reg2
	ble.b	1f					|# 符号考慮 (バッファ1 使用数 >= バッファ2 使用数) なら bra
	|#-------[ バッファ1 使用数 < バッファ2 使用数 ]
		move.w	\divy_12(%pc),%d0			|# %d0.w = \divy_12
		move.w	\divy_23(%pc),%d1			|# %d1.w = \divy_23
		sub.w	min_divh(%pc),%d1			|# %d1.w = \divy_23 - min_divh
		cmp.w	%d0,%d1
		ble.b	0f				|# 符号考慮 (\divy_12 >= \divy_23 - min_divh) なら bra
...
1:
	cmp.w	%a4,%d4
	bge.b	0f					|# 符号考慮 (バッファ1 使用数 <= バッファ2 使用数) なら bra
	|#-------[ バッファ1 使用数 > バッファ2 使用数 ]
		move.w	\divy_12(%pc),%d0			|# %d0.w = \divy_12
		move.w	\divy_01(%pc),%d1			|# %d1.w = \divy_01
		add.w	min_divh(%pc),%d1			|# %d1.w = \divy_01 + min_divh
		cmp.w	%d1,%d0				
		ble.b	0f				|# 符号考慮 (\divy_01 + min_divh >= \divy_12) なら bra
...
adjust_div_AB_CD_EF_GH:
			...
			|#-------[ バッファ AB 分割 Y 座標の更新 ]
				...
				|#-------[ バッファA 使用数 < バッファB 使用数 ]
					...
					cmp.w	%d0,%d1
					ble.b	0f				|# 符号考慮 (divy_AB >= divy_BC - min_divh) なら bra
...
adjust_div_AB_else:
				cmp.w	%a3,%d3
				bge.b	0f			|# 符号考慮 (バッファA 使用数 <= バッファB 使用数) なら bra
				|#-------[ バッファA 使用数 > バッファB 使用数 ]
					move.w	divy_AB(%pc),%d0			|# %d0.w = divy_AB
					cmp.w	min_divh(%pc),%d0			
					ble.b	0f				|# 符号考慮 (min_divh >= divy_AB) なら bra
					|#-------[ min_divh < divy_AB ]
...
0:
			...
			|#-------[ バッファ GH 分割 Y 座標の更新 ]
				...
				|#-------[ バッファG 使用数 < バッファH 使用数 ]
					...
					cmp.w	%d0,%d1				
					ble.b	0f				|# 符号考慮 (divy_GH >= XY_MAX - min_divh) なら bra
...
adjust_div_GH_else:
				cmp.w	%a6,%d6
				bge.b	0f					|# 符号考慮 (バッファG 使用数 <= バッファH 使用数) なら bra
				|#-------[ バッファG 使用数 > バッファH 使用数 ]
					move.w	divy_GH(%pc),%d0			|# %d0.w = divy_GH
					move.w	divy_FG(%pc),%d1			|# %d1.w = divy_FG
					add.w	min_divh(%pc),%d1			|# %d1.w = divy_FG + min_divh
					cmp.w	%d1,%d0				
					ble	0f				|# 符号考慮 (divy_FG + min_divh >= divy_GH) なら bra
