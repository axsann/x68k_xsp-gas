...
.macro	SORT_512b_An	An

	|#=======[ ラスタ分割バッファに登録 ]
		tst.w	(\An)			|#[12]	バッファチェック
		bmi.b	1f		|#[8,10]	負なら終点なので飛ばす
...
0:
	...
	|#-------[ 終点に達した ]
	|# end_mark:
1:	addq.w	#4,%a0			|# ポインタ補正（cd,prを飛ばす）
		dbra	%d7,SORT_512b_LOOP
		bra.b	0b
...
.macro	PR_PROTECT	buff_0_protect,buff_1_protect
					|# %d0.w = buff_0_total
					|# %d1.w = buff_1_total
	cmp.w	%d0,%d1
	bge.b	1f		|# 偶 <= 奇 なら bra
...
0:	move.w	%d0,%d7
		...
		ble.b	2f		|# \buff_1_protect >= %d7 なら bra
...
1:					|# %d1.w = buff_1_total
		...
		ble.b	2f		|# \buff_0_protect >= %d1 なら bra
...
#-------[ 初期化 3 ]
					...
					|# %d1.l = temp
	move.w	#0x1FC,%d2		|# %d2.w = SP_y 下位 4 ビット切り捨て用 and値
					|# %d3.l = #ラスタ分割バッファB
					|# %d4.l = #ラスタ分割バッファD
					|# %d5.l = #ラスタ分割バッファF
					|# %d6.l = #ラスタ分割バッファH
...
0:	add.w	%d0,(%a0)+			|# buff_X_total に 転送数*8 加算
	...
	beq.b	0f
		...
		add.w	%d1,%d1			|# %d1.w = スキップ数*16
		move.w	%d1,CHAIN_OFS_div(%a2)	|# チェイン情報の「スキップ数*8」に書込み
0:	add.w	%d0,(%a0)+			|# buff_X_totalに 転送数*8 加算
...
#-------[ 隙間詰め処理 ]
	...
	cmp.w	(%a1)+,%d7
	bge.b	0f					|# buff_X_total <= 64*8 なら bra
