...
xsp_out_entry:
#=======[ XSP 初期化チェック ]
	cmpi.b	#0b00000011,XSP_flg
	beq.b	0f			|# XSP が正しく初期化されているなら bra
...
OX_tbl_INIT_TBL:
	...
	dc.b	0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01
	dc.b	0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01
	dc.b	0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01 , 0x01,0x01,0x01,0x01 , 0x01,0x02,0x03,0xFF
...
#-------[ PR 別先頭テーブル[32].l の初期化 ]
	...
	move.l	%d0,%d7
					...
					|#	%a1.l = PR 別先頭テーブル
	lea.l	0x40*4(%a1),%a1		|#[8]	%a1.l = PR 別先頭テーブルの末端
...
#-------[ 0 なので end_mark の可能性有り ]
	move.b	#0x10,%d2			|# pr = 0 が連鎖すると end_mark を取りこぼすので無理やり補正
...
REQ_PCGDEF_v:
	...
	bvc.b	0f			|#[8,10]	V=0 なら bra
		eor.w	#0xC000,%d5	|#[ 8]	反転コード加工
0:
...
#-------[ 初期化 ]
	...
	movea.l	OX_chk_ptr(%pc),%a5	|# %a5.l = OX_tbl 検索ポインタ
					...
					|# %d2.l = temp
					|# %d3.l = temp
	move.l	#0xEB8000,%d4		|# %d4.l = #0xEB8000（下位１バイトは 0 のかわり）
...
PCG_DEF_1_L0:
	...
	bne.b	0b			|# 定義されているなら bra

	|#-------[ 空き PCG 検索 ]
0:		cmp.b	(%a5)+,%d2	|# < %d2.b なら 3 フレーム未使用
		dbhi	%d7,0b		|# 注：cc 成立でループを抜ける時、%d7.w はデクリされない
...
FOUND_PCG_1:
	...
	|#-------[ PCG 定義実行 ]
		...
		add.l	%d4,%d1			|# %d1.l = #0xEB8000 + PCG_No.* 128
						|#      = 転送先
...
PCG_DEF_2_L0:
	...
	|#-------[ 空き PCG 検索 ]
0:		cmp.b	(%a5)+,%d6	|# < %d6.b なら今回のフレームにおいて未使用
		dbhi	%d7,0b		|# 注：cc 成立でループを抜ける時、%d7.w はデクリされない
...
#-------[ 初期化 ]
	...
	lea.l	0x10*4(%a1),%a1		|# pr >= 0x10 に強制補正しているので、pr = 0x10 よりスキャン
